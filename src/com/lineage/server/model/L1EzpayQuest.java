...
public class L1EzpayQuest {
    public static final int QUEST_END = 255;
    private static final Log _log = LogFactory.getLog(L1EzpayQuest.class);
    private L1PcInstance _owner = null;
    private Map<Integer, Integer> _quest = null;

    public L1EzpayQuest(L1PcInstance owner) {
        this._owner = owner;
    }
    ...
    public int get_step(int quest_id) {
        if (this._quest == null) {
            ...
            try {
                ...
                pstm = con.prepareStatement("SELECT * FROM character_account_ezpay WHERE account_name=?");
    ...
    public void set_step(int quest_id, int step) {
        ...
        try {
            ...
            if (this._quest.get(new Integer(quest_id)) == null) {
                pstm = con.prepareStatement("INSERT INTO character_account_ezpay SET account_name = ?, quest_id = ?, quest_step = ?");
                pstm.setString(1, this._owner.getAccountName());
                pstm.setInt(2, quest_id);
                pstm.setInt(3, step);
                pstm.execute();
            } else {
                pstm = con.prepareStatement("UPDATE character_account_ezpay SET quest_step = ? WHERE account_name = ? AND quest_id = ?");
                pstm.setInt(1, step);

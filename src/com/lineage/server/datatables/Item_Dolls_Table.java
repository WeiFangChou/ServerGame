...
public class Item_Dolls_Table {
    private static final ArrayList<Integer> _idList = new ArrayList<>();
    private static Item_Dolls_Table _instance;
    private static final Log _log = LogFactory.getLog(Item_Dolls_Table.class);

    public static Item_Dolls_Table get() {
        if (_instance == null) {
            _instance = new Item_Dolls_Table();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            pstm = con.prepareStatement("SELECT * FROM `魔法娃娃合成公告系統`");
        ...
        _log.info("載入魔法娃娃合成公告數量: " + _idList.size() + "(" + timer.get() + "ms)");
    }
    ...
    private static void updata_name(int item_id) {
        ...
        try {
            ...
            ps = cn.prepareStatement("UPDATE `魔法娃娃合成公告系統` SET `名稱`=? WHERE `道具編號`=?");
            int i = 0 + 1;

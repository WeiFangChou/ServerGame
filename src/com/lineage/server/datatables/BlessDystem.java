...
public class BlessDystem {
    private static final Map<Integer, L1BlessDystem> _enchantlist = new HashMap();
    private static BlessDystem _instance;
    private static final Log _log = LogFactory.getLog(BlessDystem.class);

    public static BlessDystem get() {
        if (_instance == null) {
            _instance = new BlessDystem();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            pm = co.prepareStatement("SELECT * FROM `祝福裝備能力系統`");
            rs = pm.executeQuery();
            while (rs.next()) {
                int id = rs.getInt("序號");
                L1BlessDystem en = new L1BlessDystem();
                ...
                en.setAC(rs.getInt("防禦"));
                en.setWeight(rs.getInt("負重值"));
        ...
        _log.info("載入祝福裝備能力資料數量: " + _enchantlist.size() + "(" + timer.get() + "ms)");
    }

    public static L1BlessDystem get(int armorId) {
        try {
            if (_enchantlist.size() > 0) {
                for (int i = 0; i <= _enchantlist.size(); i++) {
                    L1BlessDystem list = _enchantlist.get(Integer.valueOf(i));
                    if (list != null && list.getArmorId() == armorId) {
                        return list;
                    }
                }
            }

...
public class ItemBsgTable {
    private static final ArrayList<Integer> _idList = new ArrayList<>();
    private static ItemBsgTable _instance;
    private static final Log _log = LogFactory.getLog(ItemBsgTable.class);

    public static ItemBsgTable get() {
        if (_instance == null) {
            _instance = new ItemBsgTable();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            pstm = con.prepareStatement("SELECT * FROM `道具中央公告系統`");
        ...
        _log.info("載入打寶中央公告物編號數量: " + _idList.size() + "(" + timer.get() + "ms)");
    }
    ...
    private static void updata_name(int item_id) {
        ...
        try {
            ...
            ps = cn.prepareStatement("UPDATE `道具中央公告系統` SET `名稱`=? WHERE `道具編號`=?");
            int i = 0 + 1;

...
public class Item_Make_Table {
    private static final ArrayList<Integer> _idList = new ArrayList<>();
    private static Item_Make_Table _instance;
    private static final Log _log = LogFactory.getLog(Item_Make_Table.class);

    public static Item_Make_Table get() {
        if (_instance == null) {
            _instance = new Item_Make_Table();
        }
        return _instance;
    }
    ...
    public void load() {
        ...
        try {
            ...
            pstm = con.prepareStatement("SELECT * FROM `製造物品公告系統`");
        ...
        _log.info("載入製造物品公告物編號數量: " + _idList.size() + "(" + timer.get() + "ms)");
    }
    ...
    private static void updata_name(int item_id) {
        ...
        try {
            ...
            ps = cn.prepareStatement("UPDATE `製造物品公告系統` SET `名稱`=? WHERE `道具編號`=?");
            int i = 0 + 1;

...
public class NewEnchantSystem {
    private static final Map<Integer, L1NewEnchantSystem> _enchantlist = new HashMap();
    ...
    public void load() {
        ...
        try {
            ...
            pm = co.prepareStatement("SELECT * FROM `武器強化能力系統`");
            rs = pm.executeQuery();
            while (rs.next()) {
                int id = rs.getInt("序號");
                L1NewEnchantSystem en = new L1NewEnchantSystem();
        ...
        _log.info("載入武器強化資料數量: " + _enchantlist.size() + "(" + timer.get() + "ms)");
    }

    public static L1NewEnchantSystem get(int weaponId, int Evolevel) {
        try {
            if (_enchantlist.size() > 0) {
                for (int i = 0; i <= _enchantlist.size(); i++) {
                    L1NewEnchantSystem list = _enchantlist.get(Integer.valueOf(i));
                    if (list != null && list.getWeaponId() == weaponId && list.getEnchant() == Evolevel) {
                        return list;
                    }
                }
            }

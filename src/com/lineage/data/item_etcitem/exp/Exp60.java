...
public class Exp60 extends ItemExecutor {
    ...
    public void execute(int[] data, L1PcInstance pc, L1ItemInstance item) throws Exception {
        if (item != null && pc != null && L1BuffUtil.cancelExpSkill(pc)) {
            int time = this._time;
            pc.setSkillEffect(L1SkillId.EXP60, time * L1SkillId.STATUS_BRAVE);
            pc.getInventory().removeItem(item, 1);
            pc.sendPackets(new S_ServerMessage("經驗質提升600%(" + this._time + "秒)。"));

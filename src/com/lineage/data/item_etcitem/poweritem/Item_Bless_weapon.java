...
import william.ItemBlessweapon;
import william.L1William_Bless_weapon;

public class Item_Bless_weapon extends ItemExecutor {
    private int chance_Bless = 0;

    private Item_Bless_weapon() {
    }

    public static ItemExecutor get() {
        return new Item_Bless_weapon();
    }
    ...
    public void execute(int[] data, L1PcInstance pc, L1ItemInstance item) throws Exception {
        ...
        if (tgItem != null) {
            if (pc.getInventory().getSize() >= 179) {
                ...
                L1William_Bless_weapon Bless_weapon_Item = ItemBlessweapon.getInstance().getTemplate(tgItem.getItem().getItemId());
                if (Bless_weapon_Item != null) {
                    int ItemId = Bless_weapon_Item.getItem_Id();
                    int enchant_level = tgItem.getEnchantLevel();
                    int EnchantKind = tgItem.getAttrEnchantKind();
                    int EnchantLevel = tgItem.getAttrEnchantLevel();
                    if (tgItem == null) {
                        return;
                    }
                    if (this.chance_Bless >= Random.nextInt(100) + 1) {
                        ...
                        item1.setAttrEnchantKind(EnchantKind);
                        item1.setAttrEnchantLevel(EnchantLevel);
                        pc.getInventory().storeItem(item1);
                        pc.sendPackets(new S_ServerMessage(166, "【" + item1.getLogName() + "】瞬間散發祝福的光芒"));
                        switch (Bless_weapon_Item.getMessage()) {
                            case 1:

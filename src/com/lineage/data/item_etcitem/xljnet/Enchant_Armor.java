...
public class Enchant_Armor extends ItemExecutor {
    private int _type = 0;

    public static ItemExecutor get() {
        return new Enchant_Armor();
    }
    ...
    public void execute(int[] data, L1PcInstance pc, L1ItemInstance item) throws Exception {
        ...
        if (tgItem.getItem().get_safeenchant() <= -1) {
        ...
        } else if (tgItem.getItem().getType2() == 0 || tgItem.getItem().getType2() == 1) {
            pc.sendPackets(new S_ServerMessage("防具才能受到保護。"));
            ...
            int use_type = tgItem.getItem().getType();
            if (use_type == 8 || use_type == 9 || use_type == 10 || use_type == 12) {
                pc.sendPackets(new S_ServerMessage("防具才能受到保護。"));
                return;
            }
            switch (this._type) {

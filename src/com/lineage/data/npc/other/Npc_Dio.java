...
public class Npc_Dio extends NpcExecutor {
    ...
    private class Work implements Runnable {
        ...
        private Work(L1NpcInstance npc) {
            this._point = new Point[]{new Point(33453, 32805), new Point(33454, 32806), new Point(33454, 32804), new Point(33453, 32803)};
            this._npc = npc;
            this._spr = SprTable.get().getMoveSpeed(npc.getTempCharGfx(), 0);
            this._npcMove = new NpcWorkMove(npc);
        }

        /* synthetic */ Work(Npc_Dio npc_Dio, L1NpcInstance l1NpcInstance, Work work) {
            this(l1NpcInstance);
        }
        ...
        public void run() {
            Point point = null;
            try {
                int t = Npc_Dio._random.nextInt(this._point.length);
                ...
                while (isWork) {
                    ...
                    if (this._npc.getLocation().isSamePoint(this._point[0])) {
                        ...
                        this._npc.setHeading(2);
                        this._npc.broadcastPacketX8(new S_ChangeHeading(this._npc));
                    } else if (this._npc.getLocation().isSamePoint(this._point[3])) {

        ...
        try {
            ...
            if (pc.isCrown()) {
                if (pc.getQuest().isStart(CKEWLv50_1.QUEST.get_id()) && npc.getNpcId() == 80135) {
                    pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "50quest_p"));
                } else {
                    QuestClass.get().startQuest(pc, CKEWLv50_1.QUEST.get_id());
                }
            } else if (pc.isKnight()) {
                if (pc.getQuest().isStart(CKEWLv50_1.QUEST.get_id()) && npc.getNpcId() == 80136) {
                    pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "50quest_k"));
                } else {
                    QuestClass.get().startQuest(pc, CKEWLv50_1.QUEST.get_id());
                }
            } else if (pc.isElf()) {
                if (pc.getQuest().isStart(CKEWLv50_1.QUEST.get_id()) && npc.getNpcId() == 80133) {
                    pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "50quest_e"));
                } else {
                    QuestClass.get().startQuest(pc, CKEWLv50_1.QUEST.get_id());
                }
            } else if (pc.isWizard()) {
                if (pc.getQuest().isStart(CKEWLv50_1.QUEST.get_id()) && npc.getNpcId() == 80134) {
                    pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "50quest_w"));
                } else {
                    QuestClass.get().startQuest(pc, CKEWLv50_1.QUEST.get_id());
                }

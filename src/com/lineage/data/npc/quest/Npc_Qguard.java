...
public class Npc_Qguard extends NpcExecutor {
    ...
    public void talk(L1PcInstance pc, L1NpcInstance npc) {
        try {
            if (pc.isCrown()) {
                pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "qguard1"));
            } else if (pc.isKnight()) {
                if (pc.getLevel() >= KnightLv30_1.QUEST.get_questlevel()) {
                    switch (pc.getQuest().get_step(KnightLv30_1.QUEST.get_id())) {
                        case 5:
                            pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "qguard"));
                            return;
                    }
                    pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "qguard1"));
                } else {
                ...
                switch (pc.getQuest().get_step(KnightLv30_1.QUEST.get_id())) {
                    case 5:
                        if (cmd.equalsIgnoreCase("teleportURL")) {
                            pc.sendPackets((ServerBasePacket)new S_NPCTalkReturn(npc.getId(), "qguards"));
                            break;
                        }
                        if (cmd.equalsIgnoreCase("teleport gerard-dungen")) {
                            staraQuest(pc);
                            isCloseList = true;
                        }
                        break;
